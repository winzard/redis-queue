...
class Channel:
    ...
    def acknowledge(self, message_id):
        self.transport.xack(self.name, self.group_name, message_id)

    def is_pending(self, message_id):
        pending_entries = self.transport.xpending_range(self.name, self.group_name, message_id, message_id,
    ...
    def ping(self, consumer_id):
        ...
        :return:
        """
        self.transport.xclaim(self.name, self.group_name, consumer_id, WORKER_TIMEOUT_MILLISECONDS, ['1'])
    ...
    def _dispatch(self, consumer_id, message_id, command) -> bool:
        ...
        pending_entries = self.transport.xpending_range(self.name, self.group_name, '-', message_id,
                                                        MESSAGE_QUEUE_SIZE)
        ...
        for message in pending_entries:  # todo time_since_delivered если застряло
            ...
            rng = self.transport.xrange(self.name, request_id, request_id, count=1)
            if not rng:
                print(f'непонятно, почему нет сообщения')
                self.transport.xack(self.name, self.group_name, request_id)
                continue
            ...
            for _command_id in entry:
                ...
                if consumer_id != other_worker.decode('utf-8') and _command_id != command_id and tenant_id == _tenant_id and \
                    ...
                    self.transport.xclaim(self.name, self.group_name, other_worker, 1, [message_id])
    ...
    def get_message_for(self, consumer_id):
        ...
        consumers = self.transport.xinfo_consumers(self.name, self.group_name)
        for info in consumers:
            ...
            if pending_messages == 0 and idle_time > WORKER_TIMEOUT_MILLISECONDS:
                ...
                self.transport.xgroup_delconsumer(self.name, self.group_name, consumer)
            if pending_messages and idle_time > COMMAND_TIMEOUT_MILLISECONDS:
                ...
                entries = self.transport.xpending_range(self.name, self.group_name, '-', '+', MESSAGE_QUEUE_SIZE,
                                                        consumername=consumer)
                message_ids = [message['message_id'] for message in entries]
                if message_ids:
                    self.transport.xclaim(self.name, self.group_name, consumer_id, COMMAND_TIMEOUT_MILLISECONDS, message_ids)
                    for f in [None, 'worker.txt', f"worker-{self.name}-{consumer_id}.txt"]:
                        logger(f,
        ...
        entries = self.transport.xpending_range(self.name, self.group_name, '-', '+', 1,
        ...
        for message in entries:  # todo time_since_delivered
            ...
            entries = self.transport.xrange(self.name, request_id, request_id)
            if entries:
                ...
                self.transport.xdel(self.name, request_id)
        print(f'новый цикл {self.last_seen}')

        entries = self.transport.xreadgroup(self.group_name, consumer_id, {self.name: self.last_seen}, count=1,
